# Visual-Specification Stage Contract

This stage owns only character/look, location/view, prop/state, voice identity, and cross-scene continuity in `剧集/<EP>/视觉设定.md`.
It inherits script facts and does not decide transient shot state, composition, action endpoints, or generation prompts.

Record each determination in the same document as reuse, a new identity, a new variant, or genuinely unresolved. Cite scene IDs as sources, and use visible headings and stable IDs for cross-document consumption. Do not create occurrences, decisions, ledgers, or acceptance records for identity determinations.
...
Rule levels, from highest to lowest, are `structural_invariant` (a blocking structural defect),
`reviewed_invariant` (requires evidentiary judgment), `craft_default` (a common practice that may be overridden),
and `taste_option` (a creator choice, not a defect). Creator-approved facts take priority over this table.
